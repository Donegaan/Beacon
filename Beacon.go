...
func attendBufferChannel() {
    for {
        ...
        if more {
            ...
            cmd := exec.Command("python3 /home/andrew/NS3DockerEmulator/docker/consumer/request_objects.py", packet.Source.String())
            err := cmd.Run()
...
func beacon() {
    ...
    if Conn != nil {
        ...
        for {
            ...
            CheckError(err)
            streamObject := myIP.String() + " " + payload.Message
            cmd := exec.Command("python3 /home/andrew/NS3DockerEmulator/docker/minimal/publish_objects.py", streamObject)
            err := cmd.Run()
